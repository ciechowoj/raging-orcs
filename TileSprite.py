...
class TileSprite:
    """Klasa reprezentuje kafel podłoża, wczytuje go z pliku podanego w konstruktorze."""
    ...
    def get_frame(self, position, time):
        """Zwraca odpowiednią ramkę animacji, zależną od pozycji i czasu."""
    ...
    def get_icon(self, size):
        """Zwraca ikone, czyli powierzchnię wyświetlaną w liście wyboru w edytorze."""
        if self._icon == None or vec2(self._icon.get_size()) != size:
            self._icon = transform.scale(self._frames[0][0][0], size.intcpl())
        return self._icon

    def get_priority(self):
        """Zwraca priorytet kafla, kafle są rysowane według priorytetu."""
        return self._priority

    def set_priority(self, priority):
        """Ustawia priorytet kafla."""
        self._priority = priority

    def get_file_name(self):
        """Zwraca nazwą pliku z którego kafel został wczytany."""
        return self._file_name

    def is_animated(self):
        """Zwraca true jeśli kafel jest animowany, false w przeciwnym razie."""
